...
class Distill_Tuning(torch.nn.Module):
    ...
    def generate_with_gate(self, prompts_ids, max_length, gate_classifier):
        """
        ...
        while cur_len <= max_length:
            # 일단 정상 샘플링 수행한다.
            outputs = self.model(input_ids=output_ids, return_dict=True)
            next_token_logits = outputs.logits[:, -1, :]
            ...
            vanilla_output_ids = torch.cat([output_ids, next_tokens.unsqueeze(1)], dim=1)
            vanilla_output_texts = self.tokenizer.batch_decode(vanilla_output_ids[:, init_len:])


            # if vanilla_output_texts.endswith("==="):
            #     output_ids = vanilla_output_ids
            #     break

            if gate_classifier(vanilla_output_texts):
                output_ids = vanilla_output_ids
                inputs_embeds = torch.cat([inputs_embeds, self.embeddings(next_tokens).unsqueeze(1)], dim=1)
                cur_len = cur_len + 1
                continue

            # if gate not pass, try discup
...
#         print("runing time is:",end-start)
    ...
    def generate(self, prompts_ids, max_length):
        """
        generation forward based on given prompt tokens, 
        Args:
            prompt_ids: the  prompt tokens
        ...
        while cur_len <= max_length:
            ...
            # vanilla_output_texts = self.tokenizer.batch_decode(output_ids[:, init_len:])
            # if vanilla_output_texts.endswith("==="):
            #     break
