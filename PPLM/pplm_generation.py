...
from typing import Optional, List, Iterable, Dict, Any, Union
...
def generate_text_pplm(
    ...
    for i in trange(length, desc='Generating with PPLM', disable=True):
        ...
        if perturb:
            ...
            pert_probs = top_k_top_p_filtering(pert_probs, top_k=top_k, top_p=top_p, is_probs=True)  # + SMALL_CONST

            # rescale
            if torch.sum(pert_probs) <= 1:
                pert_probs = pert_probs / torch.sum(pert_probs)
        else:
            pert_logits = top_k_top_p_filtering(pert_logits, top_k=top_k, top_p=top_p)  # + SMALL_CONST
            pert_probs = F.softmax(pert_logits, dim=-1)
...
class PPLMGenerator:
    ...
    top_k: int = 50
    alpha: float = 1.0
    gate_model_name: Optional[str] = None
    device: str = "cpu"
    float16: bool = False
    gate_threshold: float = 0.5

    def __post_init__(self):
        self.gen = PPLMGeneration("./models/PPLM/")
    ...
    def __call__(self, 
                 ...
                 repetition_penalty=1.0,
                 clean_up_tokenization_spaces=True,
                 include_context_in_generation=False) -> Any:
        tokenized_cond_text = self.tokenizer.encode(self.tokenizer.bos_token + prompt)
