...
gate_model_name = "../models/roberta_toxicity_classifier" # gate_model_name = "s-nlp/roberta_toxicity_classifier"
...
@torch.no_grad()
def classify_gpt2_tokens_toxic(tokens, gate_threshold):
    text = enc.decode(tokens.tolist())
    ids = gate_tokenizer(text, return_tensors="pt").to("cuda:0")
    prob = gate_classifier(**ids)[0].softmax(-1)[:, 1].item()
    return prob > gate_threshold # 0.005
...
def sample_from_hidden(model, args, classifier, context=None, past=None, device='cuda',
    ...
    for i in trange(args.length, ascii=True, disable=True):
        ...
        # not toxic
        if classify_gpt2_tokens_toxic(next_output[0], args.gate_threshold) != 1:
            if args.print_intermediate_result:
...
def run_model():
    ...
    parser.add_argument('--gate_threshold', type=float, default=0.005)
